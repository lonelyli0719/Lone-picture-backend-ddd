package com.lone.lonepicturebackend.manager.auth;
...
import com.lone.lonepicturebackend.manager.auth.model.SpaceUserPermissionConstant;
...
import com.lone.lonepicturebackend.model.enums.SpaceRoleEnum;
import com.lone.lonepicturebackend.model.enums.SpaceTypeEnum;
import com.lone.lonepicturebackend.service.PictureService;
import com.lone.lonepicturebackend.service.SpaceService;
import com.lone.lonepicturebackend.service.SpaceUserService;
...
import static com.lone.lonepicturebackend.model.constant.UserConstant.USER_LOGIN_STATE;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    /**
     * 返回一个账号所拥有的角色标识集合 (权限与角色可分开校验),本项目中不使用
     */
    @Override
    ...
    private SpaceUserAuthContext getAuthContextByRequest() {
        ...
        // 兼容 get 和 post 操作，post请求有json对象
        if (ContentType.JSON.getValue().equals(contentType)) {
            // 注意，HttpServleRequest的body值是个流，只能读取一次，读完就没了。所以需要在config包下自定义请求包装类和请求类过滤器
        ...
        if (ObjUtil.isNotNull(id)) {
            // 获取请求路径的业务前缀，如/api/picture/aaa?a=1
    ...
    /*
      编程式鉴权，在任意位置调用。基于ThreadLocal实现上下文管理的示例代码
     */
    // region
